...
for seed_inst in $(sed -n ${lstart},${lend}p ${inst_file}); do
    ...
    IFS=$' '
    echo "python3 grka.py rcpsp ${inst} --max_cpu ${max_cpu} --max_evals ${max_evals} --seed ${seed} ${params} ${params2} 2>&1 | gzip &> ${out_file}"
    python3 grka.py rcpsp ${inst} --max_cpu ${max_cpu} --max_evals ${max_evals} --seed ${seed} ${params} ${params2} 2>&1 | gzip &> ${out_file}
    IFS=$'\n'
done
